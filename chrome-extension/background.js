...
// 标签页刷新策略
let tabRefreshInterval = 2;   // 每 N 次 solve 刷新一次
let solveCount = 0;
let lastTabRefresh = Date.now();
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  if (alarm.name === 'keepAlive') {
    ...
    // 每 5 分钟定时刷新标签页，防止页面老化
    const now = Date.now();
    if (captchaTabId !== null && (now - lastTabRefresh) > 5 * 60 * 1000) {
      addLog('INFO', '定时刷新标签页（5 分钟周期）');
      await refreshCaptchaTab();
    }
  }
});
...
// 刷新标签页：重新加载页面，重置 reCAPTCHA 行为信号
async function refreshCaptchaTab() {
  if (captchaTabId === null) return;
  try {
    await chrome.tabs.reload(captchaTabId);
    // 等待页面加载完成
    ...
    lastTabRefresh = Date.now();
    addLog('SUCCESS', `标签页刷新完成 (tabId=${captchaTabId})`);
  } catch (e) {
    addLog('WARN', `标签页刷新失败: ${e.message}，将重新创建`);
    captchaTabId = null;
  }
}
...
async function processSolve(msg) {
  ...
  // solve 完成后检查是否需要刷新标签页
  solveCount++;
  if (solveCount >= tabRefreshInterval) {
    addLog('INFO', `已累计 ${solveCount} 次请求，刷新标签页...`);
    solveCount = 0;
    await refreshCaptchaTab(); // 队列内同步刷新，后续请求会等待
  }
}
