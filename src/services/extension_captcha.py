...
class ExtensionCaptchaService:
    ...
    @max_consecutive_errors.setter
    def max_consecutive_errors(self, value: int):
        self._max_consecutive_errors = max(0, value)
    ...
    def set_client_enabled(self, client_id: int, enabled: bool) -> bool:
        """启用/禁用指定客户端（手动操作时重置自动禁用标记和连续失败计数）"""
        info = self._clients.get(client_id)
        if info is None:
            return False
        info.enabled = enabled
        if enabled:
            # 手动启用时重置自动禁用标记和连续失败计数
            info.auto_disabled = False
    ...
    async def get_token(
        ...
        try:
            ...
            self._check_auto_disable(client)
    ...
    def _check_auto_disable(self, client: ClientInfo):
        """检查连续失败次数，达到阈值时自动禁用客户端"""
        client.consecutive_errors += 1
        if (self._max_consecutive_errors > 0 and
                client.consecutive_errors >= self._max_consecutive_errors):
            client.enabled = False
            client.auto_disabled = True
            logger.warning(
                f"[ExtCaptcha] 客户端 {client.client_id} (name={client.name}) "
                f"连续失败 {client.consecutive_errors} 次，已自动禁用"
            )
    ...
    async def test_client(
        ...
        Returns:
            {"success": True/False, "token_length": N, "elapsed_ms": N, "error": "..."}
        """
        info = self._clients.get(client_id)
        if info is None:
            return {"success": False, "error": "客户端不存在"}
        if not info.authed:
            return {"success": False, "error": "客户端未认证"}
